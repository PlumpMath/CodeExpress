...
;; Create a data structure for a Push state based upon the currently defined push types.
...
(def specification-table (atom (hash-map)))

(defmacro define-registered
  "Register an instruction with the interpreter.
instruction is a symbol.
specification is a map that describes the function, including I/O specs.
definition is a function that maps from pushstate to pushstate."
  [instruction specification definition]
  `(do (register-instruction '~instruction)
     (swap! specification-table assoc '~instruction ~specification)
