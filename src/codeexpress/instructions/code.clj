(ns codeexpress.instructions.code
...
(define-registered 
  code_do*count
  {:in {:code 1 :integer 1}; note: missing nuance to integer
   :out {:exec 1}}
  (fn [state]
...
(define-registered 
  exec_do*count
  {:in {:exec 1 :integer 1}
   :out {:exec 1}};; differs from code.do*count only in the source of the code and the recursive call    
  (fn [state] 
    (if (not (or (empty? (:integer state))
                 (< (first (:integer state)) 1)
                 (empty? (:exec state))))
...
(define-registered 
  exec_do*times
  {:in {:exec 1 :integer 1}
   :out {:exec 1}};; differs from code.do*times only in the source of the code and the recursive call
  (fn [state]
    (if (not (or (empty? (:integer state))
                 (< (first (:integer state)) 1)
                 (empty? (:exec state))))
...
(define-registered code_fromboolean
  {:in {:boolean 1}
   :out {:code 1}}
  (codemaker :boolean))
(define-registered code_fromfloat
  {:in {:float 1}
   :out {:code 1}}
  (codemaker :float))
(define-registered code_frominteger
  {:in {:integer 1}
   :out {:code 1}}
  (codemaker :integer))
(define-registered code_quote
  {:in {:exec 1}
   :out {:code 1}}
  (codemaker :exec))
...
(define-registered exec_noop {:in {} :out {}} (fn [state] state))
(define-registered code_noop {:in {} :out {}} (fn [state] state))
